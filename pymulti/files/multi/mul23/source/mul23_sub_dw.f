...
C
      SUBROUTINE WEQW
C
C  WRITES THE EQUIVALENT WIDTHS TO FILE WEQ
C  WRITES THE POPULATION DENSITIES TO FILE WN
C
C: WEQW   91-12-18  MODIFICATIONS: (MATS CARLSSON)
C:        INCLUDED COMMON BLOCK CDW, PRINTOUT OF SELECTED INDICES
C:
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATOM'
...
C
      ICALL=ICALL+1
      IF(ICALL.EQ.1) THEN
        CALL MOPEN(LDW,'WEQ',1,'NEW')
        CALL MOPEN(LDN,'WN',1,'NEW')
        WRITE(LDN,100) NASEL
        WRITE(LDW,100) NASEL
        DO 110 KR=1,NASEL
          WRITE(LDN,100) IRAD(KRSEL(KR)),JRAD(KRSEL(KR))
          WRITE(LDW,100) IRAD(KRSEL(KR)),JRAD(KRSEL(KR))
  100     FORMAT(2I5)
  110   CONTINUE
        WRITE(LDN,100) NFSEL
        WRITE(LDW,100) NFSEL
        DO 120 KR=1,NFSEL
          WRITE(LDN,100) KRSEL(KR+NASEL)
          WRITE(LDW,100) KRSEL(KR+NASEL)
  120   CONTINUE
        WRITE(LDN,100) NCSEL
        WRITE(LDW,100) NCSEL
        DO 130 KR=1,NCSEL
          WRITE(LDN,100) ICSEL(KR),JCSEL(KR)
          WRITE(LDW,100) ICSEL(KR),JCSEL(KR)
  130   CONTINUE
      ENDIF
C
      WRITE(LDW,200) (WEQ(KR),KR=1,NLINE)
  200 FORMAT(3P,5F15.6)
      DO 310 K=1,NDEP
        WRITE(LDN,300) (N(I,K),I=1,NK)
  300   FORMAT(1P,6E15.7)
  310 CONTINUE
C
      RETURN
      END
...
C***********************************************************************
C
      SUBROUTINE CHRATE(ISTOP)
C
C  CHANGES RATES BY A FACTOR OF FAC ONE BY ONE
C
C: CHRATE 91-03-05  MODIFICATIONS: (MATS CARLSSON)
C:        COLLISIONAL RATES THAT ARE ZERO ARE NOT PERTURBED
C:
C:        91-06-27  MODIFICATIONS: (MATS CARLSSON)
C:        READ IN RATES TO BE CHANGED FROM FILE CHRATE
C:
C:        91-12-17  MODIFICATIONS: (MATS CARLSSON)
C:        ONLY CHANGED RATES ARE OUTPUT
C:
C:        91-12-18  MODIFICATIONS: (MATS CARLSSON)
C:        INCLUDED COMMON BLOCK CDW
C:
C:        08-04-09  MODIFICATIONS: (MATS CARLSSON)
...
C:
      ...
      INCLUDE 'CDW'
C
      SAVE ICALL,AOLD,BOLD,ACOLD,COLD,FOLD
      DIMENSION AOLD(MLINE),BOLD(MK,MK),ACOLD(MQ,MWIDE),
     * COLD(MK,MK,MDEP),FOLD(MK,MK,MDEP)
      DATA ICALL/0/
C
      ICALL=ICALL+1
C
C  ON FIRST CALL, STORE
C  COLLISIONAL RATES AND OTHER FIXED RATES.
C  SET WRITE PARAMETERS TO 0 EXCEPT IWEQW
C
      IF(ICALL.EQ.1) THEN
        FAC=2.
        ISTOP=0
        DO 400 K=1,NDEP
          DO 300 J=1,NK
            DO 200 I=1,NK
              COLD(I,J,K)=COL(I,J,K)
              FOLD(I,J,K)=FIX(I,J,K)
  200       CONTINUE
  300     CONTINUE
  400   CONTINUE
C  
C  SAVE A AND B VALUES FOR ALL LINES, ALFAC FOR CONTINUA
C
        DO 410 KR=1,NLINE
          AOLD(KR)=A(KR)
  410   CONTINUE
C
        DO 420 KR=NLINE+1,NRAD
          DO 415 NY=1,NQ(KR)
            ACOLD(NY,KR-NLINE)=ALFAC(NY,KR-NLINE)
  415     CONTINUE
  420   CONTINUE
C
        DO 440 J=1,NK
  ...
  440   CONTINUE
C
C  SET IW PARAMETERS TO 0 EXCEPT IWEQW
C
        IWHEAD=0
        IWATOM=0
        IWABND=0
      ...
      ELSE
C
C  RESET VALUES LAST CHANGED.
C
  450   CONTINUE
        IF(ITYPE(ICALL-1).EQ.0) THEN
          KR=KRSEL(ICALL-1)
          IF(KR.LE.NLINE) THEN
C
C  LAST CHANGE WAS A LINE
C
            I=IRAD(KR)
            J=JRAD(KR)
            A(KR)=AOLD(KR)
          ...
          ELSE
C
C  LAST CHANGE WAS A CONTINUUM
C
            DO 500 NY=1,NQ(KR)
              ALFAC(NY,KR-NLINE)=ACOLD(NY,KR-NLINE)
  500       CONTINUE
          ENDIF
        ELSE IF(ITYPE(ICALL-1).EQ.1) THEN
C
C  LAST CHANGE WAS A FIXED RATE
C
          KF=KRSEL(ICALL-1)
          I=IFX(KF)
          J=JFX(KF)
          DO 550 K=1,NDEP
            C(I,J,K)=COLD(I,J,K)+FOLD(I,J,K)
            C(J,I,K)=COLD(J,I,K)+FOLD(J,I,K)
  550     CONTINUE
        ELSE
C
C  LAST CHANGE WAS COLLISIONAL RATE
C
  560     CONTINUE
          I=ICSEL(ICALL-1-NASEL-NFSEL)
          J=JCSEL(ICALL-1-NASEL-NFSEL)
          DO 580 K=1,NDEP
            C(I,J,K)=COLD(I,J,K)+FOLD(I,J,K)
            C(J,I,K)=COLD(J,I,K)+FOLD(J,I,K)
  580     CONTINUE
      ...
      ENDIF
C
C  TEST TO SEE IF WE ARE DONE
C
      IF(ICALL.GT.NSEL) THEN
        ISTOP=1
        RETURN
      ENDIF
C
C  CHANGE RATE
C
      IF(ITYPE(ICALL).EQ.0) THEN
        KR=KRSEL(ICALL)
        IF(KR.LE.NLINE) THEN
C
C  CHANGE A LINE. A VALUE AND B VALUES
C
          I=IRAD(KR)
          J=JRAD(KR)
          A(KR)=AOLD(KR)*FAC
          ...
          B(J,I)=BOLD(J,I)*FAC
          WRITE(*,590) KR,I,J
  590     FORMAT(' CHANGING BB TRANSITION, KR=',I4,'  (',I4,'-',I4,')')
        ELSE 
C
C  CHANGE A CONTINUUM. ALFAC VALUES
C
          DO 600 NY=1,NQ(KR)
            ALFAC(NY,KR-NLINE)=ACOLD(NY,KR-NLINE)*FAC
  600     CONTINUE
          WRITE(*,610) IRAD(KR)
  610     FORMAT(' CHANGING BF TRANSITION, I=',I4)
        ENDIF
      ELSE IF(ITYPE(ICALL).EQ.1) THEN
C
C  CHANGE A FIXED RATE
C
        KF=KRSEL(ICALL)
        I=IFX(KF)
        J=JFX(KF)
        DO 650 K=1,NDEP
          C(I,J,K)=COLD(I,J,K)+FOLD(I,J,K)*FAC
          C(J,I,K)=COLD(J,I,K)+FOLD(J,I,K)*FAC
  650   CONTINUE
        WRITE(*,660) KF,I,J
  660   FORMAT(' CHANGING FIXED TRANSITION, KF=',I4,'  (',I3,'-',I3,')')
      ELSE
C
C  CHANGE A COLLISIONAL RATE. 
C
        I=ICSEL(ICALL-NASEL-NFSEL)
        J=JCSEL(ICALL-NASEL-NFSEL)
        DO 680 K=1,NDEP
          C(I,J,K)=COLD(I,J,K)*FAC+FOLD(I,J,K)
          C(J,I,K)=COLD(J,I,K)*FAC+FOLD(J,I,K)
  680   CONTINUE
        WRITE(*,690) I,J
  690   FORMAT(' CHANGING COLLISIONS  (',I4,'-',I4,')')
      ENDIF
C
      RETURN
      END
...
C
      SUBROUTINE CHREAD
C
C  READS INDICES FOR RATES TO PERTURB
C
C: CHREAD 91-12-18  NEW ROUTINE: (MATS CARLSSON)
C:
C:        93-07-23  MODIFICATIONS: (MATS CARLSSON)
C:        ITYPE INDEX CORRECTED FOR CASE NASEL.LT.0
C:
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATOM'
      ...
      INCLUDE 'CDW'
C
C  READ FILE CHRATE TO FIND WHAT RATES TO PERTURB
C  NASEL IS THE NUMBER OF DETAILED RATES TO PERTURB WITH INDICES IN KRSEL
C  NFSEL IS THE NUMBER OF FIXED RATES TO PERTURB WITH INDICES IN KRSEL
C  NCSEL IS THE NUMBER OF COLLISIONAL RATES TO PERTURB, INDICES IN 
C        ICSEL AND JCSEL
C  NSEL  IS TOTAL NUMBER OF RATES TO PERTURB
C  ITYPE IS TYPE OF RATE TO PERTURB (0=DETAILED, 1=FIXED, 2=COLLISIONAL)
C
C  IF NASEL.LT.0 ALL NON-ZERO RATES WILL BE PERTURBED
C
      CALL CSTRIP(LCHA,'CHRATE')
      NSEL=0
      READ(LDUMS,*) NASEL
      IF(NASEL.LT.0) THEN
        NASEL=NRAD
   ...
   20   CONTINUE
        ...
        DO 40 I=1,NK-1
          DO 30 J=I+1,NK-1
            IF(C(I,J,NDEP).EQ.0.0) GOTO 30
            NCSEL=NCSEL+1
            ICSEL(NCSEL)=I
            JCSEL(NCSEL)=J
            ITYPE(NCSEL+NASEL+NFSEL)=2
   30     CONTINUE
   40   CONTINUE
        NSEL=NASEL+NFSEL+NCSEL
      ELSE
        DO 50 KR=1,NASEL
          READ(LDUMS,*) I,J
          IF(KRAD(I,J).EQ.0) THEN 
            WRITE(LJOBLO,'(A,2I3)') 'I,J=',I,J
            CALL STOP('CHREAD: NO SUCH TRANSITION')
          ENDIF
          KRSEL(KR)=KRAD(I,J)
          NSEL=NSEL+1
          ITYPE(NSEL)=0
   ...
   60   CONTINUE
        ...
        DO 70 KR=1,NCSEL
          READ(LDUMS,*) I,J
          IF((I.LE.0) .OR. (J.LE.0) .OR. (I.GT.NK) .OR. (J.GT.NK)) THEN
            CALL STOP('CHREAD: INVALID VALUES OF I,J FOR CSEL')
          ENDIF
          ICSEL(KR)=I
          JCSEL(KR)=J
          NSEL=NSEL+1
