...
C:        CHANGED TO USE 3-DIAGONAL RYBICKI-HUMMER OPERATOR
C:
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATOM'
...
C
      DIMENSION XTEMP(MDEP),D1(MDEP),E1(MDEP),ALI(MDEP,-2:2)
...
C
  ...
  420 CONTINUE
C
      ALI(1,0)=1./(1.-D1(1)*E1(2))/(SP21-SP31)
      ALI(NDEP,0)=1./(SP2N-SP1N*(1.-D1(NDEP-1)))
      DO 440 K=2,NDEP-1
        ALI(K,0)=XTEMP(K)/(1.-D1(K)*E1(K+1))
  440 CONTINUE
C
c
c  calculate off diagonal lambda elements
c  subtract diagonal element
c
      do 350 k=2,ndep-1
        ali(k,-1)=e1(k)*ali(k-1,0)
        ali(k,+1)=d1(k)*ali(k+1,0)
  350 continue
      ali(1,1)=d1(1)*ali(2,0)
      do 360 k=1,ndep-1
        ali(k,0)=ali(k,0)-1.
  360 continue
c
      DO 550 M=-1,1
        KK0=MAX(1-M,1)
        KK1=MIN(NDEP-M,NDEP-1)
        DO 500 K=KK0,KK1
          DW = WC*ALFA(K)*Z(K) * ALFA(K+M)/(X(K+M)*XNORM(K+M))
          W(I,K,J,K+M) = W(I,K,J,K+M) + GIJ(K+M)*DW*ALI(K,M)*
     *     (IPLUS(K+M) + IMINUS(K+M) + 2.*HN3C2)
          W(J,K,I,K+M) = W(J,K,I,K+M) + DW*ALI(K,M)*
     *     (IPLUS(K+M) + IMINUS(K+M))
  500   CONTINUE
  550 CONTINUE
C
