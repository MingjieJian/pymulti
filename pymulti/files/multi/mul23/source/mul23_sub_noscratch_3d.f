...
C:
C:        04-03-08  MODIFICATIONS: (MATS CARLSSON)
C:        MPI VERSION FOR 3D COLUMNS
C:
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATOM'
...
C
      ...
      GRAV=10.0**GRAVLG
C
c calculate h populations from rho and lte
C
      HLTE=.false.
      dpid='3d'
      dpcon1=-6
...
C
     ...
     *   exp(-13.6d0*ee/bk/temp(k))
        ...
        nh(6,k)=ratio/(1.d0+ratio)*totnh
        ratio=ne(k)/phit
        nh(1,k)=ratio/(1.d0+ratio)*totnh
        DO 430 I=2,5
          NH(I,K)=i*i*exp(-13.6d0*(1.d0-1.d0/i/i)*ee/bk/temp(k))*
     *      nh(1,k)
  430   CONTINUE
  500 CONTINUE
C
...
C:
      ...
      INCLUDE 'CATOM'
C
      DIMENSION TMX(MDEP),DP(MDEP),dpnew(mdep)
      CHARACTER*10 FMT
C
...
c  pass new depthscale to caller instead of writing to file
C
c      CALL MOPEN(LDSCA2,'DSCAL2',1,'NEW')
c      WRITE(LDSCA2,150) DPID(1:61)
c  150 FORMAT('* DEPTH SCALE FROM DSCAL2'/' DSCAL2 ON ',A)
c      IF(DPTYPE.EQ.'M') THEN
c        WRITE(LDSCA2,160)
c  160   FORMAT(' MASS SCALE')
c        DDP=0.5E-6
c        WRITE(FMT,'(A)') '(0P,F15.6)'
c      ELSE IF(DPTYPE.EQ.'H') THEN
c        WRITE(LDSCA2,165)
c  165   FORMAT('HEIGHT SCALE')
c        DDP=-0.5E-6*DP(1)
c        WRITE(FMT,'(A)') '(1P,E15.6)'
c      ELSE
c        WRITE(LDSCA2,170)
c  170   FORMAT(' TAU5000 SCALE')
c        DDP=0.5E-6
c        WRITE(FMT,'(A)') '(0P,F15.6)'
c      ENDIF
c      WRITE(LDSCA2,210) NDEP,DPCON1
c  210 FORMAT(I4,F15.6)
c      WRITE(LDSCA2,FMT) DP(1)+DDP
      dpnew(1)=dp(1)
      T=0.
      L=1
      DO 300 K=2,NDEP-1
  ...
  250   L=L+1
        ...
        dpnew(k)=cm2
c        WRITE(LDSCA2,FMT) CM2
        L=L-1
  300 CONTINUE
       dpnew(ndep)=dp(ndep)
c      WRITE(LDSCA2,FMT) DP(NDEP)-DDP
c      CALL CLOSE(LDSCA2)
C
      RETURN
      END
...
C
C: FREQ   MODIFICATIONS: (MATS CARLSSON)
C:        ONLY CALL ONCE IF 3D AND IATOM=0
C:
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATOM'
      ...
      DATA FCALL/.TRUE./
C
      IF(.NOT.FCALL .AND. IATOM2.EQ.0) RETURN
      FCALL=.FALSE.
C
...
C:        DPTYPE='H' ADDED
C:
C:        06-12-18  MODIFICATIONS: (MATS CARLSSON)
C:        MPI VERSION FOR 3D COLUMNS
C:
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
...
C
      IF(ISTART.LE.-1) THEN
C
C  start approximation has already been read from n3d file, skip reading
C
c        read(lrstrt) n(1:nk,1:ndep)
C
C  GET THE SCATTERING CONTRIBUTION TO THE SOURCE FUNCTION CONSISTENT 
C  WITH THE RADIATION FIELD
C
...
C
     ...
     *  'IDL1  ','IDLNY ','IDLCNT','IOSMET','EOSMET','END   '/
...
C     EPSILON FOR LINE OPACITY. EPSILON=NNE/(EOSMET+NNE)
C
...
C  DEFAULT VALUES
C
      CALL DEFVAL(VNAME,CVALUE,MVAR,'IWARN ','2')
...
C     03-01-21 REMO COLLET: ADDED IOSMET VARIABLE TO THE LIST
      ...
      CALL DEFVAL(VNAME,CVALUE,MVAR,'EOSMET','1.E15')
C
...
C
  ...
  740 CONTINUE
     ...
     *   IWLTE,IWN,IWNIIT,IWOPAC,IWRAD,IWRATE,
     *   IWSTRT,IWTAUQ,IWTEST,IWWMAT,IWJFIX,IWARN,
     *   IDL1,IDLNY,IDLCNT,IOSMET,EOSMET
C     03-01-21 REMO COLLET: ADDED IOSMET VARIABLE TO THE LIST.
...
C
  ...
  310         CONTINUE
            ENDIF
C
            if(kr.eq.krsave .and. ny.eq.nusave) xsave(1:ndep)=x(1:ndep)
...
C
      ...
      CHARACTER*10 STAT,CMPMYID
      CHARACTER*132 FILE2
      DATA LU1/20/,ICALL0/0/
      SAVE ICALL0
C
      ICALL0=ICALL0+1
C
...
C
  ...
  100   CONTINUE
        ...
        DO 110 I = 1,22
          MDATA(I)=I
  110   CONTINUE
        WRITE(CMPMYID,'(I4)') mpmyid2
        K0=1
        CALL GETWRD(CMPMYID,K0,K1,K2)
        FILE2='TEST_'//CMPMYID(K1:K2)
        OPEN(LU1,FILE=FILE2,FORM='UNFORMATTED',ACCESS='SEQUENTIAL',
     &   STATUS='UNKNOWN')
C
...
C
        ...
        CLOSE(LU1)
        OPEN(LU1,FILE=FILE2,FORM='UNFORMATTED',ACCESS='DIRECT',
...
C  DO NOT ASSIGN NUMBERS 100-102
C
      DO 200 I = LU1,MAXLU
        IF(I.EQ.100 .OR. I.EQ.101 .OR. I.EQ.102) GOTO 200
...
C
      ...
      ENDIF        
C
C  IF NEW FILE, APPEND EXTENSION
C
c      IF(STAT.EQ.'NEW' .OR. STAT.EQ.'UNKNOWN') THEN
c        FILE2=FILE(1:LENT(FILE))//EXT2(1:LENT(EXT2))
c      ELSE
        FILE2=FILE
c      ENDIF
C
C  IF MYNEW OR MYOLD, APPEND MPMYID
C
      IF(STAT0.EQ.'MYNEW' .OR. STAT0.EQ.'MYOLD') THEN
        WRITE(CMPMYID,'(I4)') mpmyid2
        K0=1
        CALL GETWRD(CMPMYID,K0,K1,K2)
        FILE2=FILE(1:LENT(FILE))//'_'//CMPMYID(K1:K2)
        IF(STAT0.EQ.'MYNEW') THEN
          STAT=NEWSTA
        ELSE
          STAT='OLD'
        ENDIF
      ELSE IF(STAT0.EQ.'MASTERNEW') THEN
        STAT=NEWSTA
        FILE2=FILE
      ENDIF 
C
c      write(*,'(a,i3,1x,a)') 'open ',iunit,file2(1:lent(file2))
      IF(IRECW.EQ.0) THEN
        OPEN(IUNIT,FILE=FILE2,FORM='UNFORMATTED',STATUS=STAT)
      ELSE IF(IRECW.EQ.1) THEN
        OPEN(IUNIT,FILE=FILE2,FORM='FORMATTED',STATUS=STAT)
      ELSE IF(IRECW.GT.1 .AND. STAT(1:3) .NE. 'OLD') THEN
        OPEN(IUNIT,FILE=FILE2,ACCESS='DIRECT',RECL=IRC*IRECW,
     *  FORM='UNFORMATTED',STATUS=STAT)
      ELSE IF(IRECW.LT.0 ) THEN
        OPEN(IUNIT,FILE=FILE2,ACCESS='DIRECT',RECL=-IRCSP*IRECW,
     *  FORM='UNFORMATTED',STATUS=STAT)
...
C
C  READS MEAN INTENSITIES FROM ARRAY CJ
C
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATMOS'
...
C
      IF(IREC.GT.MCJ) THEN
        ...
        DO 50 KR=1,NRAD
          ICJ=ICJ+NQ(KR)
   50   CONTINUE
        WRITE(LJOBLO,'(A,I10)') ' MCJ SHOULD BE .GE.',ICJ
        CALL STOP('READJ: IREC.GT.MCJ')
      ENDIF
      DO 100 K=1,NDEP
        JNY(K)=CJ(K,IREC)
  100 CONTINUE
...
C
      IF(ICP.GT.MCP) THEN
        ...
        DO 50 KR=1,NLINE
          ICJ=ICJ+NQ(KR)
   50   CONTINUE
        WRITE(LJOBLO,'(A,I10)') ' MCP SHOULD BE .GE.',ICJ*MMU
        CALL STOP('READP: ICP.GT.MCP')
      ENDIF
      DO 100 K=1,NDEP
        PHI(K)=CP(K,ICP)
  100 CONTINUE
...
C
      SUBROUTINE READX
C
C  READS OPACITIES FROM ARRAY CX
C
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATMOS'
...
C
      IF(ICX.GT.MCX) THEN
        ...
        DO 50 KR=1,NRAD
          ICJ=ICJ+NQ(KR)
   50   CONTINUE
        WRITE(LJOBLO,'(A,I10)') ' MCX SHOULD BE .GE.',ICJ
        CALL STOP('READX: ICX.GT.MCX')
      ENDIF
      DO 100 K=1,NDEP
        XCONT(K)=CX(K,ICX)
  100 CONTINUE
      DO 200 K=1,NDEP
        SCAT(K)=CX(K+NDEP,ICX)
  200 CONTINUE
      DO 300 K=1,NDEP
        SC(K)=CX(K+NDEP+NDEP,ICX)
  300 CONTINUE
      ICX=ICX+1
C
      RETURN
...
C
C  REWINDS FORTRAN UNIT LU OR SETS RECORD POINTER IN ARRAYS CX AND CP
C
      INCLUDE 'PARAM'
      INCLUDE 'CLU'
      INCLUDE 'CXJP' 
...
C
C  WRITES MEAN INTENSITIES TO ARRAY CJ
C
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATMOS'
...
C
      SUBROUTINE WRITEP
C
C  WRITES PROFILE PHI TO ARRAY CP
C
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATMOS'
...
C
      ...
      SAVE ICALL
C
C  IF FIRST CALL, WRITE FIRST ''RECORD''
C
      ICALL=ICALL+1
      IF(ICALL.EQ.1) ICP=1
C
...
C
      SUBROUTINE WRITEX
C
C  WRITES OPACITIES TO ARRAY CX
C
      INCLUDE 'PREC'
      INCLUDE 'PARAM'
      INCLUDE 'CATMOS'
...
C
   ...
   50   CONTINUE
        WRITE(LJOBLO,'(A,I10)') ' MCX SHOULD BE .GE.',ICJ
        CALL STOP('WRITEX: ICX.GT.MCX')
      ENDIF
      DO 100 K=1,NDEP
        CX(K,ICX)=XCONT(K)
  100 CONTINUE
      DO 200 K=1,NDEP
        CX(K+NDEP,ICX)=SCAT(K)
  200 CONTINUE
      DO 300 K=1,NDEP
        CX(K+NDEP+NDEP,ICX)=SC(K)
  300 CONTINUE
      ICX=ICX+1
C
      RETURN
